...
class WC_Trelis_Gateway extends WC_Payment_Gateway
{
	...
	public function trelis_subscription_status_updated($subscription, $new_status, $old_status)
	{
		...
		if ($new_status == "pending-cancel") {
			...
			$order->add_order_note(__("Cancel Subscription Request added.", 'trelis-crypto-payments'), true);
		}
		...
		if ($new_status == 'active' && $old_status == 'pending-cancel' || $new_status == 'active' && $old_status == 'on-hold') {
			...
			// wp_mail( 'ronan@trelis.com', 'Trelis run subscription API status changed to Active', print_r($response,true), $headers );
